...
static int __apic_accept_irq_ib(struct kvm_lapic *apic, int delivery_mode,
			     int vector, int level, int trig_mode,
			     struct dest_map *dest_map);
...
int kvm_apic_set_irq_ib(struct kvm_vcpu *vcpu, struct kvm_lapic_irq *irq,
		     struct dest_map *dest_map)
{
	struct kvm_lapic *apic = vcpu->arch.apic;

	return __apic_accept_irq_ib(apic, irq->delivery_mode, irq->vector,
			irq->level, irq->trig_mode, dest_map);
}
...
bool kvm_irq_delivery_to_apic_fast_ib(struct kvm *kvm, struct kvm_lapic *src,
		struct kvm_lapic_irq *irq, int *r, struct dest_map *dest_map)
{
	struct kvm_apic_map *map;
	unsigned long bitmap;
	...
	*r = -1;
	...
	ret = kvm_apic_map_get_dest_lapic(kvm, &src, irq, map, &dst, &bitmap);
	// printk("kvm_irq_delivery 1!,bitmap is %lu\n",bitmap);
	if (ret) {
		*r = 0;
		for_each_set_bit(i, &bitmap, 16) {
			if (!dst[i])
				continue;
			*r += kvm_apic_set_irq_ib(dst[i]->vcpu, irq, dest_map);
			// printk("kvm_irq_delivery 2!\n");
		}
	}
